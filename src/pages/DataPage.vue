              ...
              <div class="d-flex align-items-center">
                <nav><ul class="main-nav-ul" style="display: flex !important;">
                  ...
                  <li  v-for="(item) in getDbTables" :key="item.table_name"
                       ...
                       class="breadcrumb-item main-nav-li-item" >{{item.table_name}}</li>
        ...
        <div class="container-fluid" style="margin:2px; padding:2px;" >
          ...
          <div style="margin:0px 0px 0px 4px; padding: 0px; display: flex">
            <button @click="beforyAddItem()"
                    class="btn btn-outline-info" type="button" style="border-radius: 0px"> Предварительная запись </button>
            <button @click="addItem()"
                    class="btn btn-outline-info" type="button" style="border-radius: 0px; margin-left: 10px"> Добавить новую запись </button>
          </div><hr>
              ...
              <div class="data-list-center-panel">
                ...
                <template v-if="beforyNewItems.length" >
                      <div class="bw" style="padding:0px;">
                           <button @click="saveNewItems()" class="btn btn-outline-info animeCustomBtn" style="border-radius: 0px;"> Сохранить новые данные</button>
                      </div>
...
<!--                    <h5 class="card-subtitle">Данные</h5>-->

                    <button @click="showVertical = !showVertical"
                            class="btn btn-outline-info" type="button" style="border-radius: 0px; margin-left: 10px"> Изменить вид данных </button> </p>
                    ...
                    <template v-else >
                        <table v-if="tableData.length"
                               class="tablesaw table-bordered table-hover table no-wrap tablesaw-swipe" style="cursor:pointer;" >
                          <thead>
                          <tr>
                            <th  v-for="(item, fname) in tableInfo" scope="col" class="border"
                                 :title="item.data_type +'-||-'+ item.column_default" >{{fname}}</th>
                            <th></th>
                          </tr>
                          </thead>
                          <tbody>

    ...
    <div class="row">
      <div class="col-lg-12 col-md-8">
        ...
        <div class="card"><div class="card-body" >
              <div v-if="user_name">
                  <span style="font-style: italic;">Выбранный пользователь : </span>
                  <span style="color:green;margin-left:10px;" >{{user_name}}</span>
              </div>
...
<!--        <pre>{{getUserItemInfo}}</pre>-->
    ...
    <div class="row" >
        ...
        <div class="col-lg-6 col-md-8">
          ...
          <!----- ПРОСМОТР ПОЛЕЙ ПОЛЬЗОВАТЕЛЯ    ------->
          <div v-if="user_name"  class="card"><div class="card-body">
               <ShowItemInfo
                   title="Информация о пользователе"
                   :item_info="getUserItemInfo"
               ></ShowItemInfo>
          </div></div>
        ...
        <div class="col-lg-6 col-md-4" >
              ...
              <div class="card"><div class="card-body">
                  ...
                  <h6 class="card-title my-custom-card-title" style="font-size: 15px; font-weight: bolder; margin-top:30px;">Прикрепленные права к базам</h6>
                  <div v-for="(item, i) in attachedDbList" :key="item.name">
                      <div >{{item.name}}</div>
                  </div>
                  ...
                  <div><pre v-if="showToggleJson" >{{ getUserItemInfo }}</pre></div>
...
export default {
  ...
  methods: {
    ...
    getAttachedDbList() {
        ...
        let items = [];
        for(let i in this.getDbList) {
            let db = this.getDbList[i];
            if(!db.datacl) continue;

            let name = db.datname;
            let datacl = db.datacl;

            let num = datacl.indexOf(userName);
            if(num !== -1)
               items.push({ name, num});
        }
